...
- **Pôsteres**: cada painel de trabalho pode ter um `<div class="sheet">` no fim, com um `<figure>` por produção. Célula recortada em 2:3, **em cor cheia desde o repouso**; no hover a moldura interna acende no acento e a imagem clareia de leve. O número de colunas é fixo por faixa (2 até 559px, 3 até 860px, 5 acima), nunca `auto-fill`, para não sobrar fileira órfã com um pôster só; a célula cresce com a tela, de 165px no celular a 221px no desktop, com teto de 1180px no `.sheet`. Pôster de produção com material público é link para o mesmo destino do `.plink` do texto; sem material, entra como `<figure class="mute">` sem link. Arquivos e nomes em `assets/posters/README.md`.
